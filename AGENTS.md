...
**Branch**: `develop` (features merge here from `feature/*`).
...
**Canonical `mvnw`**: `app/mvnw`. Stale duplicate at `app/uam/mvnw` — do not use it.

**Order matters**: Spotless `check` is bound to `verify`. Run `spotless:apply` first, or `verify` fails.
...
- **Interface-first controllers**: interface declares `@GetMapping` etc.; `@RestController` impl adds `@RequestMapping` with full path (e.g. `/uam/v1/user`).
...
- **Required header**: All endpoints require `App-Correlation-Id`.
...
- **Custom repo method**: `UserRepository.findByIdAndVersion(UUID, Integer)` — used by update/delete for optimistic locking.
...
| Method | Path | Service |
|---|---|---|
| `GET` | `/uam/v1/hello-world/greeting` | `HelloWorldService` |
| `POST` | `/uam/v1/user/register` | `RegisterUserService` |
| `GET` | `/uam/v1/user` | `SearchUserService` |
| `GET` | `/uam/v1/user/{userId}` | `GetUserService` |
| `PATCH` | `/uam/v1/user/{userId}` | `UpdateUserService` |
| `DELETE` | `/uam/v1/user/{userId}` | `DeleteUserService` |

Query params: `name`, `pageNo` (default 1), `totalRecordsPerPage` (default 10) on search; `version` on update/delete.
...
- **Nested payload pattern**: Every request carries a `@Valid @NonNull` inner `Payload` static class. Exceptions: `UpdateUserReqDTO` has `userId`/`version` at DTO level alongside the payload; `DeleteUserReqDTO` nests `userId`/`version` inside the payload.
...
- **Service tests**: `@SpringBootTest` (full context). In-memory H2 (`jdbc:h2:mem:uamdb`) via `application-test.yml`. Test profile activated by `src/test/resources/application.properties`.
- **Common module**: no tests exist — `src/test/` absent; new tests must be created from scratch.
...
- **Liquibase**: `db/db.changelog-master.xml` → `db/changelog/db.changelog-t_user_260620.xml` (creates `T_USER` with JPA auditing fields).
- **Main config**: file-based H2 (`jdbc:h2:./h2/portal`; `AUTO_SERVER=TRUE`), JPA `ddl-auto: validate`.
- **Prod config** (`application-prod.yml`): env-var-driven (`UAM_DB_USERNAME`, `UAM_DB_PASSWORD`, `UAM_DB_URL`).
- **Test config**: in-memory H2 (`jdbc:h2:mem:uamdb`).
...
- **EditorConfig**: 2-space indent for all, LF endings, UTF-8.
...
- **Dev container**: Docker Compose (`mcr.microsoft.com/devcontainers/java:3-25-trixie`). OpenCode pre-installed via devcontainer feature.
- **Logback**: Writes to `./logs/uam.log` (relative to `app/uam/`), archived daily with 100MB size cap, 30-day retention, 10GB total cap. Gitignored via root `.gitignore`.
